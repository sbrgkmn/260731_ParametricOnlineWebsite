...
export function Marketplace({ products }: { products: MarketplaceProduct[] }) {
  ...
  const hasFreeDownloads = products.some(
    (product) => product.access === "Free" && product.status === "available",
  );
    ...
    <>
      ...
      <div className="marketplace-grid">
        {visibleProducts.map((product, index) => {
          const downloadable = product.status === "available" && Boolean(product.downloadUrl);
          const purchasable = product.status === "available" && Boolean(product.checkoutUrl);
